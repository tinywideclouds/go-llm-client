...
func (s *FirestoreSessionStore) SaveCompiledCache(ctx context.Context, firestoreCacheID urn.URN, cache *builder.CompiledCache) error {
	...
	// Hardened: encode the URN to avoid "InvalidArgument ... lacks /" errors
	safeDocID := tools.EncodeFirestoreID(cache.ID)

	ref := s.client.Collection(s.bundleCol).
		Doc(firestoreCacheID.String()).
		Collection(CompiledCachesSubCol).
		Doc(safeDocID)
...
func (s *FirestoreSessionStore) ListCompiledCaches(ctx context.Context, firestoreCacheID urn.URN) ([]builder.CompiledCache, error) {
	...
	iter := s.client.Collection(s.bundleCol).
		Doc(firestoreCacheID.String()).
		Collection(CompiledCachesSubCol).
		Documents(ctx)
	...
	for {
		...
		var fsCache firestoreCache
		if err := doc.DataTo(&fsCache); err != nil {
			s.logger.Warn("Failed to unmarshal compiled cache", "docID", doc.Ref.ID, "error", err)
			continue
		}

		// Hardened: decode the storage key back into our native domain URN
		id, err := tools.DecodeFirestoreCompiledCacheID(doc.Ref.ID)
		if err != nil {
...
func (s *FirestoreSessionStore) SaveProposal(ctx context.Context, proposal *builder.ChangeProposal) error {
	ref := s.client.Collection(LlmSessionsCollection).
		Doc(proposal.SessionID.String()).
		Collection(ProposalsSubCol).
		Doc(proposal.ID)
...
func (s *FirestoreSessionStore) GetProposalsBySession(ctx context.Context, sessionID urn.URN) ([]builder.ChangeProposal, error) {
	...
	iter := s.client.Collection(LlmSessionsCollection).
